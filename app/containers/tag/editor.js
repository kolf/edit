...
@connect(select)
export default class ZhEditTagContainer extends Component {
    ...
    componentWillMount() {
        document.title= '关键词管理 - 编辑类 - 内容管理系统 - VCG © 视觉中国';
    };
        ...
        return (
            <div className="main-content-inner">
                ...
                <div className="page-content">
                    ...
                    <Table columns={columns} dataSource={dataSource} loading={loading} pagination={paginationInit}
    ...
    queryListFindAll(params) {
        ...
        const {dispatch} = this.props;
        const data = {name: name, type: "tagEditor"}
        this.setState({loading: true});
    ...
    tagDelete(record) {
        const submit = () => {
            const {dispatch} = this.props;
            dispatch(tagDelete({type: 'cfp', id: record.id})).then(res => {
                if (res.apiError) {
