...
import { isElite, decode, getDevice} from "app/utils/utils";
...
export default class EditAllContainer extends Component {
    constructor(props) {
        ...
        this.state = {
            "crumbs": [
                {"path": "/home", "text": "首页"},
                {"path": "/zh/edit/all", "text": "编辑类：全部资源"}
            ],
            "typeDoc": {
            ...
            "alert": {},
            "filterInit": {},
            "dataFilter": filterData.list(26, 3, 4, 39, 40, 41 ,5, 6, 7, 8, 9),
            "filterParams": {
            ...
            <div className="main-content-inner">
                <div className="page-content">
                    ...
                    <SearchBox
                        ...
                        resGroup={1}
                        hotSearchs={[{label: '涉华审核',key: 'isShehua'},{label: '每日精选',key: 'bestDay'}]}
                        pageId={10}
                        ...
                        <div className="col-xs-7">
                            ...
                            {filterParams.resGroup == 1 && <span className="ml-10">
                                <Button title="复制并新建组" size="large" shape="circle" icon="addfolder"
                                        onClick={this.operateGroup.bind(this,{ operate: "copy" }) } />
                                <Button title="合并并新建组" size="large" shape="circle" icon="folder"
                                        onClick={this.operateGroup.bind(this, { operate: "merge" }) } />
                                <Button title="复制并加入组照" size="large" shape="circle" icon="laptop"
                                        onClick={this.operateGroup.bind(this, { operate: "groupCopy" }) } />
                                <Button title="拆分并加入组照" size="large" shape="circle" icon="folder-open"
                                        onClick={this.operateGroup.bind(this, { operate: "groupMerge" }) } />
                                <Button title="添加到待编审" size="large" shape="circle" icon="addfile" className="ml-10"
                                        onClick={this.addToNoEdit.bind(this) }></Button>
                                <Button title="添加到未编审" size="large" shape="circle" icon="file-unknown"
                                        onClick={this.addToTheEdit.bind(this) }></Button>
                                <Button title="加入专题" size="large" shape="circle" icon="select" className="ml-10"
                                        onClick={this.addToTopic.bind(this) }></Button>
                            </span>}
                            {filterParams.resGroup == 2 && <span className="ml-10">
                                <Button title="复制并新建组" size="large" shape="circle" icon="addfolder"
                                        onClick={this.operateGroup.bind(this,{ operate: "imageCopyNew" }) } />
                                <Button title="拆分并新建组" size="large" shape="circle" icon="folder"
                                        onClick={this.operateGroup.bind(this, { operate: "imageSplitNew" }) } />
                                <Button title="复制并加入组照" size="large" shape="circle" icon="laptop"
                                        onClick={this.operateGroup.bind(this, { operate: "imageCopyGroup" }) } />
                                <Button title="拆分并加入组照" size="large" shape="circle" icon="folder-open"
                                        onClick={this.operateGroup.bind(this, { operate: "imageSplitMerge" }) } />
    ...
    refresh(type, dataParams) {
        ...
        // console.log(type, dataParams,filterParams);
    ...
    handleOnThumbnail({operate, key, id, resId, groupId, value, groupIds}) {
        switch (operate) {
            case "edit": // 编审
                ...
                let url = "/zh/doing/group/edit/" + id + "?groupId=" + groupId
            ...
            case "addToTheEdit": // 添加到未编审
                ...
                break;
            case "push": // 推送
                if(id && groupId) window.open("/zh/doing/group/push/" + id + "?groupId=" + groupId);
                break;
            case "viewGroup":
                if(id && groupId) window.open('/zh/group/'+id +'?groupId='+groupId);
                break;
            ...
            case "accessLink": // 前台链接
                ...
                break;
            case "editGroup": // 前台链接
                if (id && groupId) window.open('/zh/group/update/' + id + '?groupId=' + groupId);
                break;
    ...
    handleOnSearch(params, dataFilter, type) {
        ...
        // console.log(onlineState, resGroup, agency, onlineTime)
        if(!type && type !== 'rss'){
    ...
    operateGroup({operate, key, id}) {
        ...
        const hasMultpleGroupIds = (ids) => {
            return listSelectData.list.reduce((result, item) => {
                if(item.groupIds && item.groupIds.length>1){
                    result.push(item.resId)
                }
                return result;
            }, []).join(',');
        }

        const hasEliteIds = (ids) => {
            return listSelectData.list.reduce((result, item) => {
                 if (item.topicIds && isElite(_.pluck(item.topicIds, 'id'))) {
                    result.push(item.resId)
                 }
                 return result;
            }, []).join(',')
        ...
        const ids = listSelectData.ids.map(id => id*1);
        ...
        if(hasEliteIds(ids)){
            this.messageAlert({title, "body": "每日精选组照不能新建、合并、复制、拆分，请取消选择！", "type": "error"});
            return false;
        }

        if(/imageSplit/.test(operate)){
            const errorIds = hasMultpleGroupIds(ids);
            if(errorIds){
                this.messageAlert({title, "body": '单张'+errorIds+'存在于多个组照中，不能拆分，请将其取消选择！', "type": "error"});
                return false;
            }
        }
        ...
        if (type == 'group') {
            ...
            paramsAlert.list = listSelectData.list.reduce((result, item) => {
                const {id, resImageId} = item.groupIds[0];
                result.push({
                    groupId: id ? id+'' : '',
                    resImageId
                });
        ...
        if(['groupCopy', 'groupMerge', 'imageSplitMerge'].indexOf(operate)!=-1){
        ...
        if(/groupCopy|groupMerge|imageCopyGroup|imageSplitMerge/.test(operate)){
            const openAlert = () => {
                ...
                this.openAlert(Object.assign(alertProps, {
                    contentShow: 'body',
                    body: <div>
                        <p className="alert alert-info text-center"><i className="ace-icon fa fa-hand-o-right bigger-120"></i> {paramsAlert.text}</p>
                        <Input placeholder='请输入组照ID' value={alert.params.groupId} onChange={(e) => {
        ...
        const addToNoEditor = (submitType) => {
            ...
            dispatch(groupAddToNoEditor({"groupIds": ids.join(','), oneCategory})).then(result => {
                ...
                if (submitType == "submitAndEdit" && ids.length==1) {
                    window.open("/zh/doing/group/edit/" + ids[0] + "?groupId=" + ids[0]);
                }
    ...
    viewEditHistory({key, id, groupId, resId}) {
        ...
        dispatch(groupViewEditHistory(params,groupId?null:true)).then(result => {
            ...
            this.openAlert({
                ...
                "body": <EditRecord head={[{"field": "createdTime","text": "编审时间"},{"field": "userName","text": "编审人"},{"field": "message","text": "编审内容", isHtml: true}]} Lists={result.apiResponse.reverse().slice(0, 20) }/> //记录太多倒序取20个
            });
        });
    };
