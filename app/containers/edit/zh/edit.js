...
export default class EditorEditContainer extends Component {
        ...
        dispatch(getEditData(params)).then((result) => {
            ...
            list.map((item, i) => {
                ...
                if (item.keywordsAudit) {
                    item.keywordsAuditArr = item.keywordsAudit.replace(',||0|0', '').match(keywordsAuditReg).map(source => {
                        const tagArr = source.split('|')
                        keywordsTagArr = keywordsTagArr.concat(tagArr[1].split(keywordsSplitReg))
                        return {
                            label: tagArr[0],
                            id: tagArr[1],
                            type: tagArr[2],
    ...
    clearListPeopleTag() {
        ...
        list.forEach(item => {
            if (item.hide || item.qualityRank == 5 || item.offlineMark == 2) return;
            // console.log(item.keywordsAuditArr)
                                            ...
                                            if (step == 1) {
                                                if (operate === 'edit' || operate === 'editen') {
                                                    return <span>
                                                        ...
                                                        <Button className={"ml-20"}
                                                            onClick={this.operateGroup.bind(this, { operate: "addToOverseas" })}>
                                                            添加到海外
                                                                </Button>
        ...
        dispatch(findKeyword({ name: labelArr, type })).then(result => {
            ...
            labelArr.forEach((label, i) => { //
                ...
                if (!matchedKeywords || !matchedKeywords.length) {//新词按照格式加入keywordsAuditArr
                    ...
                    sortTag[matchedKeywords[0].cnname] = sortTag[matchedKeywords[0].cnname] || now(i);
